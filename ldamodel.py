...
class TopicModel(object):
    ...
    def __init__(self, model_path, dict_path, num_topics = None, vocab_size=None):
    ...
    def _gen_new_word_topic_prob(self):
        top_topics_with_words = self.topic_model.show_topics(num_topics = self.num_topics, num_words = self.vocab_size, formatted=False)
    ...
    def tau_calc(self, article):
        W = self.get_word_topic_prob()
        relevant_topics = set(self._topic_to_top_topic.keys())
        T = np.mat([topic_prob for topic_id, topic_prob in self.doc2topics(article) if topic_id in relevant_topics])
        tau_d = T*W
        return tau_d